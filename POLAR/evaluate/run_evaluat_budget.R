...
source("evaluate/evaluate_budget_run_TAR_2017.R")
...
years_summary <- summary_results_even_budget %>%
  mutate(year = "2017", .before = 0) %>%
  mutate(budget = "even", .after = 1)


summary_results_proportional_budget %>%
  select(run, recall, recall_at_k, auc, map, wss_at_r95, rfbu, ug_at_budget) %>%
  arrange(desc(recall), run) %>%
  xtable()

summary_results_proportional_budget <- summary_results_proportional_budget %>%
  mutate(year = "2017", .before = 0) %>%
  mutate(budget = "proportional", .after = 1)
  

years_summary <- years_summary %>%
  bind_rows(summary_results_proportional_budget)


summary_results_inverse_proportional_budget %>%
  select(run, recall, recall_at_k, auc, map, wss_at_r95, rfbu, ug_at_budget) %>%
  arrange(desc(recall), run) %>%
  xtable()


summary_results_inverse_proportional_budget <- summary_results_inverse_proportional_budget %>%
  mutate(year = "2017", .before = 0) %>%
  mutate(budget = "inverse", .after = 1)


years_summary <- years_summary %>%
  bind_rows(summary_results_inverse_proportional_budget)


summary_results_capped_proportional_budget %>%
  select(run, recall, recall_at_k, auc, map, wss_at_r95, rfbu, ug_at_budget) %>%
  arrange(desc(recall), run) %>%
  xtable()


summary_results_capped_proportional_budget <- summary_results_capped_proportional_budget %>%
  mutate(year = "2017", .before = 0) %>%
  mutate(budget = "capped", .after = 1)


years_summary <- years_summary %>%
  bind_rows(summary_results_capped_proportional_budget)
...
source("evaluate/evaluate_budget_run_TAR_2018.R")
...
summary_results_even_budget <- summary_results_even_budget %>%
  mutate(year = "2018", .before = 0) %>%
  mutate(budget = "even", .after = 1)


years_summary <- years_summary %>%
  bind_rows(summary_results_even_budget)
...
ggplot(data = years_summary_, aes(x = budget, y = ug_at_budget, fill = budget)) +
...
years_summary_factor$budget <- factor(
  ...
  levels = c("even", "proportional", "inverse", "capped")
)
...
ggp <- ggplot(data = years_summary_factor, aes(x = budget, y = rfbu, fill = budget)) +
  ...
  facet_wrap(~ year) +
  labs(
    title = "RFBU by Budget Allocation Strategy",
    x = "Budget Allocation Strategy",
    y = "RFBU"
  ) +
  theme_minimal(base_size = 13) +
...
ggp
ggsave("rfbu_budget_allocation.pdf", 
       plot = ggp, 
       width = 12, height = 8)
